...
class SuperProductGroupsController extends FrameworkBundleAdminController
{
   ...
   */
  private function saveOrUpdateGroup(?int $groupId, string $name, ?string $imagePath, int $groupOrder, int $superProductId): int
  {
    $db = \Db::getInstance();
    ...
    if ($groupId) {
      ...
      if (!empty($groupOrder)) {
        $updateData['group_order'] = pSQL($groupOrder);
      }
